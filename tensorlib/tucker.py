...
def hosvd(X, rank=None, dims=None, dtype=None, compute_core=True, compact=False, with_eigvals=False):
    U = [None for _ in range(X.ndim)]
    eigvals = [None for _ in range(X.ndim)]
    ...
    if dtype is None:
        dtype = X.dtype
    if rank is None:
        rank = [None for dim in dims]
    for d in dims:
        if with_eigvals:
            U[d], eigvals[d] = nvecs(X, d, rank[d], compact=compact, with_eigvals=True)
        else:
            U[d] = nvecs(X, d, rank[d], compact=compact)
    if compute_core:
        core = X.ttm(U, transp=True)
        if with_eigvals:
            return U, core, eigvals
        else:
            return U, core
    else:
        if with_eigvals:

...
from .pyutils import from_to_without
...
class dtensor(tensor_mixin, np.ndarray):
    ...
    def copy(self):
        """
        Creates and returns a deep copy of the dense tensor
        """
        return dtensor(np.copy(self))

    def _ttm_compute(self, V, mode, transp):
        """
        Tensor times matrix product in dimension 'mode'
        """
...
class unfolded_dtensor(np.ndarray):
    ...
    def copy(self):
        """
        Creates and returns a deep copy of the unfolded dtensor
        """
        return unfolded_dtensor(np.copy(self), self.mode, self.ten_shape)

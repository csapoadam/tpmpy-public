...
def nvecs(X, n, rank=None, do_flipsign=True, dtype=float, tol=1e-10, compact=True, with_eigvals=False):
    """
    ...
    if rank is not None:
        eigvals, U = eigh(Y, subset_by_index=(N - rank, N - 1))
    else:
        eigvals, U = eigh(Y)
    ...
    # filter eigenvalues and eigenvectors based on the tolerance
    if compact:
        eigvals = eigvals[::-1]
        valid_indices = eigvals > tol
        U = U[:, valid_indices]
